...
'''
    ...
    # Initialize PhotoBoothEngine.
    pbengine = PhotoBoothEngine(updateURLSignal)
    
    pbengine.on_change_url.connect(on_change_url)
    #pbengine.on_status.connect(appLabel.rootObject().findChild(QObject, "screen").item.status)
    #pbengine.on_update_filter_preview.connect(appLabel.rootObject().findChild(QObject, "screen").item.updateImageFilterPreview)
